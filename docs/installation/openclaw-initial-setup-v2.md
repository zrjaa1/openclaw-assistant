...
从 2026.3.2 版本开始，新安装的小龙虾默认使用「消息模式」（messaging profile），只能收发消息。这已经是一个安全的起点，但你可以根据需要调整到更适合自己的权限级别。

如果你是老版本升级过来的用户，你的小龙虾可能仍在使用「完全权限」模式，建议按照下面的说明调整。

**我们提供 3 个权限级别：**

**1. 安全模式（Safe Mode）— 新手推荐**

最安全的模式。小龙虾只能「看」不能「动」：可以阅读文件、搜索网页、通过聊天工具给你发消息，但不能创建或修改任何文件，也不能执行命令。

注意：安全模式下小龙虾无法保存新的记忆。也就是说每次新对话它都会"忘记"之前聊过的内容（已有的记忆仍可读取）。如果你觉得长期记忆很重要，请选择下面的读写模式。
...
在安全模式的基础上，小龙虾还可以创建和修改文件、保存记忆（记住你告诉它的重要事情）。但仍然不能执行终端命令或运行程序。
...
没有任何限制。小龙虾可以执行任何命令、控制浏览器、管理设备。老版本（2026.3.2 之前）的默认设置。
...
> 💡 **提示：** 以后想调整权限？随时可以重新运行上面的命令来切换模式。不确定选哪个？先选「安全模式」，需要更多功能时再升级。
