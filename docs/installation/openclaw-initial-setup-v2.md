...
小龙虾默认使用「完全权限」模式，意味着它可以不受限制地操作你的电脑（执行命令、读写文件、控制浏览器等）。大多数用户不需要这么大的权限。
...
没有任何限制。小龙虾可以执行任何命令、控制浏览器、管理设备。这是默认设置。

⚠️ 除非你完全了解这些权限的含义，否则不建议使用。如果你想保持默认，可以跳过这一步，但请确保你清楚小龙虾将拥有完全的操作权限。
